...
def super_attack(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    if user_id not in allowed_usersvip:
        update.message.reply_text('Đây Là Lệnh Của Người Dùng Vip . Hãy Mua Để Có Thể Dùng Nếu Mua Liên Hệ @JustAlexJonhson')
        return
    update.message.reply_text('Vui lòng nhập số điện thoại:')
...
def get_key(update: Update, context: CallbackContext):
    ...
    api_token = '60946204a27475a037aa8e78f2b3feb4826deb3e83285ab50672f9243a990881'
...
def main():
    updater = Updater(token='7312603599:AAFmbL1uhYot69TRb0SdJhtOWbGV1UMtkpU', use_context=True)
